...
**唯一的例外是结尾的问句。** AI 几乎每次都会在末尾追一句「你想进一步了解哪方面的
内容呢？」，这句对搜图毫无价值，出现频率又高，值得单独处理。判据只有位置和标点：

* 只从**最后一行**往前扫，遇到不是问句的立刻停 —— 正文中间的问号不受影响
* 按**句**切开，一行里前半是正文、后半才是提问时只砍后半（`他拥有粉色长发。你想
  了解更多吗？` → 只留前一句）

问号是标点不是措辞，所以这条规则对语言和说法都不敏感 —— 回答语言由 Google 按出口
IP 判定，同一张图换个节点就从中文变英文，凡是匹配话术的方案都会跟着失效。而描述
一张图片的正文是客观陈述，不会以问号收尾。

代价是句号或感叹号收尾的邀请语留得住，例如「请告诉我你接下来想了解的内容。」，以及
「如果你想了解更多，我可以为你提供：」后面跟着的那几条建议 —— 它们最后一行是列表项，
不是问句。这是明确接受的取舍：与其为了这几行去赌措辞，不如把误删正文的风险降到零。
