...
这类容器本身没有盒子、`offsetParent` 天然为 null，但里面的内容是可见的。用它做
过滤条件（还配上 `FILTER_REJECT`，会砍掉整棵子树）的结果是 746 个元素只剩 25 个，
正文一个字都取不到。改用 `checkVisibility()`，并且只在叶子块这一层判、用
`FILTER_SKIP` 不用 `REJECT`。

**不能排除 `<a>` 里的文本。** 一开始为了滤掉引用卡片，跳过了所有 `<a>` 内的文本，
结果首句被切成三段 —— 因为作品名是**内联在句子里的链接**。而追问建议是**整行就是
一个链接**。这两者必须区分：收集所有链接的文字，只在「一整行恰好等于某个链接文字」
时才丢。

**容器选错会把卡片一起带进来。** `[data-subtree="aimc"]` 是整块回答（3442 字），
包含底部引用卡片；`[data-subtree="aimfl"]`（首句）往上最近的 `[data-container-id]`
才是正文（3032 字），差值正好是那堆卡片。优先级搞反了就会混进十几行卡片标题。

**逐个元素抓不如直接取容器的 `innerText`。** 后者天然保持句子完整（内联链接不会被
拆出来）、天然跳过隐藏内容、表格单元格之间给 `\t`（换成「：」就是「字段：值」）。
逐块遍历还要自己处理嵌套去重，不划算。

**卡片区一旦开始就到底了，所以遇到卡片特征直接收尾**，不用逐行过滤。卡片行的特征：
相对时间后缀（`· 8 years ago`）、绝对日期开头（`2023年7月17日 —`）、裸域名、
`全部显示`。

**引导语要覆盖敬语。** 只写「如果你对」会漏掉「如果**您**对」，那次实测三条追问
建议全都漏了出来。

**AI 会拒答，而且不稳定。** 同一张图，一次给出完整的作品信息，含发行
日期，另一次回「抱歉，我无法提供此图片中相关内容的详细信息」。拒答文案没有信息量，
命中 `_AI_REFUSAL_RE` 且短于 200 字就当作没有描述，让输出只保留完全匹配。

**仍未解决**：正文中间会夹一行来源站点名（`手机新浪网`、`FANZA`、
`DARLING in the FRANXX Wiki`），是 Google 给该句标注的引用出处。它们和正文小标题
（`角色基本信息`）在形态上无法区分 —— 都是短行、无句读。按短行过滤会连带删掉小
标题，信息损失更大，所以保留，每次通常只有一行。
