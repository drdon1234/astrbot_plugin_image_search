...
**这是旧实现里 Docker 容器“抽到 20 张卡片却返回 0 条结果”的根因。**

CDP 附加的上下文不会把浏览器的 `--proxy-server` 转给 `context.request` ——
那些请求由 Playwright 进程直连发出。容器里没有系统代理，于是全部超时；而
旧实现的 `_resolve()` 会丢掉没还原出地址的条目，最终表现就是 0 条。当前实现会汇总
还原结果：候选非空但全部失败时返回明确抓取错误，只有部分失败时才保留成功项继续输出。
...
当前实现把配置代理传给跳板还原、外部图片下载和标题补全，不进行本地 DNS/IP
预检或目标网络范围过滤，因此 Clash Fake-IP 不会在请求进入代理前被误判。跳板还原
遇到站外 `Location` 时只返回地址，不访问目标站点；图片与标题请求使用普通 HTTP
重定向。流式大小限制与整次请求总时限继续保留，它们只控制资源消耗。

这一选择意味着插件没有应用层 SSRF 防护：LLM 工具提供的图片 URL 可以访问进程或
代理可达的内网与云元数据服务。部署时必须由代理 ACL、容器网络或防火墙承担隔离，
并限制函数工具的调用范围。

历史症状很有迷惑性 —— 不报错、不弹验证码、日志干净，只是结果为空。修复后无需再靠
耗时猜测；错误会直接指出候选链接还原失败。
...
用户，多几行一眼能忽略，少了正文却看不出来，所以宁可多给。本地定向测试保留一组
对照断言：同一份 HTML 里，网格前的「冒号引导语 + 列表」必须保留，网格后的必须丢弃。
...
| --- | --- |
| 20~30s | 正常（浏览器路径） |
| 明确提示候选链接还原失败 | 检查代理、DNS 与 Google 跳板请求是否可达 |
...
脚本清单见 [ARCHITECTURE.md 5.2](ARCHITECTURE.md#52-管理员排查脚本)。定位问题时
优先用 `tools/diagnose.py` 分段报告哪一环坏了；浏览器组合验证属于本地测试资产，
保存在不跟踪的 `test/` 中。
